...
class MockSQLBuilderProvider @JvmOverloads constructor(
    ...
    override fun execute(
        ...
        validate(sqlBuilder)
        checkTag(executeTag)
        if (executeSupplier == brokenExecuteSupplier) {
    ...
    @Throws(SQLException::class)
    private fun getCr(): CallResult {
        invocations.getCr++
        var cr = mockCallResults.poll()
        when {
            cr != null -> checkTag(cr.toString())
            generateSingleRowResultSet -> cr = MockCallResult(
                "",
                arrayOf(THE_ANSWER_TO_THE_ULTIMATE_QUESTION.toString()),
                autoGenerated = true
            )
            else -> cr = MockCallResult("", arrayOf(""))
        }
        logger.debug { "Using mock ResultSet $cr" }
        return cr
    }

    private fun checkTag(tag: String) {
        if (enforceTags && tag.isNotEmpty() && !tag.startsWith(MockResultSet.tagPrefix) && !tag.startsWith(MockCallResult.tagPrefix)) {
    ...
    companion object {
        ...
        private val mockCallResults: Queue<CallResult> = ConcurrentLinkedQueue()
        ...
        private val brokenExecuteSupplier: Supplier<Int> = Supplier { -1 }
        ...
        @JvmStatic
        fun addCallResult(cr: CallResult) {
            mockCallResults.add(cr)
        }
        ...
        @JvmStatic
        fun setExecuteBroken(tag: String) {
            val count = AtomicInteger()
            executeTag = tag
            executeSupplier = brokenExecuteSupplier
        }
        ...
        fun reset() {
            ...
            if (mockCallResults.isNotEmpty()) {
                logger.warn { "Unused mock ResultSet objects: ${mockCallResults.map { obj: CallResult -> obj.toString() }}" }
                mockCallResults.clear()
            }
